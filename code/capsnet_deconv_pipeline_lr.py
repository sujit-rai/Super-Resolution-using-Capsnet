...
data_path = "/home/sujit/Downloads/DIV2K_train_LR"
filename_list = []
...
X = tf.cast(tf.train.shuffle_batch([inpu], 65, 100, 90, enqueue_many=True, shapes=[14,14,3],num_threads=1),tf.float32)
...
with tf.name_scope("decoder"):
    hidden1 = tf.layers.batch_normalization(tf.layers.conv2d_transpose(decoder_input, 128, [3,3], strides = (1,1), padding = "VALID",
                              activation=tf.nn.relu,
                              name="hidden1"))
    hidden2 = tf.layers.batch_normalization(tf.layers.conv2d_transpose(hidden1, 64, [3,3], strides = (1,1), padding = "VALID",
                              activation=tf.nn.relu,
                              name="hidden2"))
    hidden3 = tf.layers.batch_normalization(tf.layers.conv2d_transpose(hidden2, 32, [3,3], strides = (1,1), padding = "VALID",
                              activation=tf.nn.relu,
                              name="hidden3"))
    hidden4 = tf.layers.batch_normalization(tf.layers.conv2d_transpose(hidden3, 16, [3,3], strides = (1,1), padding = "VALID",
                              activation=tf.nn.relu,
                              name="hidden4"))
    hidden5 = tf.layers.batch_normalization(tf.layers.conv2d_transpose(hidden4, 8, [2,2], strides = (1,1), padding = "VALID",
                              activation=tf.nn.relu,
                              name="hidden5"))
    decoder_output = tf.layers.conv2d_transpose(hidden5, 3, [1,1], strides = (1,1), padding = "VALID",
                                     activation=tf.nn.relu,
                                     name="decoder_output")
...
checkpoint_path = "/home/sujit/capsnet/checkpoint/modeldeconv_pipelined_corrected_lr1.ckpt"
...
with tf.Session() as sess:
    ...
    for iteration in range(1, n_iterations):
        ...
        if(iteration%30 == 0):
            ...
            scipy.io.savemat('W1.mat', mdict={'org': org,'reconstructions': reconstructions})
